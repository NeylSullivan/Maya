...
def TransferJointWeights(oldJointName, newJointName):
    ...
    print ''
    print 'TransferJointWeights: Transfering weights from {0} to {1}'.format(
        oldJointName, newJointName)
    for skinClusterName in skinList:
        ...
        if oldJointName not in influences:
            print 'TransferJointWeights: ' + skinClusterName + \
                ' is NOT influenced by SOURCE joint ' + oldJointName + ' Skipping...'
            continue
        elif newJointName not in influences:
            print 'TransferJointWeights: ' + skinClusterName + \
                ' is NOT influenced by TARGET joint ' + newJointName + ' Skipping...'
            continue
        else:
            print 'TransferJointWeights: ' + skinClusterName + \
                ' is influenced by joint ' + oldJointName + ' Processing...'

        cmds.skinCluster(skinClusterName, e=True,
                         selectInfluenceVerts=oldJointName)
        ...
        if onlyVertices is None:
            ...
            for v in onlyVertices:
                oldJointWeight = cmds.skinPercent(
                    skinClusterName, v, transform=oldJointName, query=True)
                newJointWeight = cmds.skinPercent(
                    skinClusterName, v, transform=newJointName, query=True)
        ...
        print 'TransferJointWeights: Removing {0} from influences of {1}'.format(
            oldJointName, skinClusterName)
...
def DestroyMiddleJoint(jointName):
    ...
    if not cmds.objExists(jointName):
        print 'DestroyMiddleJoint: joint {0} is not exist: Aborting'.format(
            jointName)
        return
    ...
    if children is not None:
        for child in children:
            print 'DestroyMiddleJoint: parenting {0} to {1}'.format(
                child, parent[0])
...
def DestroyJointChildren(jointName):
    ...
    if not cmds.objExists(jointName):
        print 'DestroyJointChildren: joint {0} is not exist: Aborting'.format(
            jointName)
        return
    ...
    if children is None:
        print 'DestroyJointChildren: joint {0} does not has children: Aborting'.format(
            jointName)
        return
